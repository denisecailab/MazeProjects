...
def PCAthroughTime(
    projections, samplingRate=1 / 30, color="salmon", saveSeries=False, path="."
):
    """
    Parameters
    ==========
    ...
    for i in range(projections.shape[1] - 1):
        try:
            ...
            ax.scatter(
                ...
                c="green",
                s=100,
            )  # start timepoint as green star
            if isinstance(color, str):
                scatterThroughTime = ax.scatter(
                    projections[0, : i + 1],
                    projections[1, : i + 1],
                    projections[2, : i + 1],
                    ".",
            ...
            ax.scatter(
                ...
                c="darkred",
                s=30,
            )  # mark current point as dark red

            # Add current time and coordinates
            timepatch = mpatches.Patch(
                color="white",
                label=("Time: " + str(np.round(i * samplingRate, decimals=3)) + "sec"),
            )
            coordpatch = mpatches.Patch(
                color="white",
                label="Coord: "
                + str(np.round(projections[0, i + 1], decimals=3))
                + ","
            ...
            if saveSeries:
                fig.savefig(
                    path + "/" + str(i).zfill(len(str(projections.shape[1]))) + ".png",
                    dpi=80,
                )
...
def runPCA(
    ...
    for n, x in enumerate(numProjections):
        projections[n, :] = np.dot(
            rates, evectors.T[x]
        )  # projections of principal components onto firing rates
    ...
    if plotEigenvectors:
        ...
        for txt in range(evectors.shape[1]):
            plt.text(
                y=-len(numVectors) / 2,
                x=np.arange(evectors.shape[1])[txt],
                s=unitnames[txt],
                fontsize=7,
        ...
        plt.title(
            "Eigenvectors for first " + str(len(numVectors)) + " PCs", fontsize=30
        )
        plt.show()
