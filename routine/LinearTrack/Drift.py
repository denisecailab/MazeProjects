...
class Drift:
    def __init__(self, mice):
        self.lt_data = MultiAnimal(
            mice,
            project_name="LinearTrack",
            SessionFunction=CalciumSession,
            session_types=session_types["lineartrack"],
        )

        self.circle_data = MultiAnimal(
            mice, project_name="RemoteReversal", SessionFunction=BehaviorSession
        )
    ...
    def scatter_box(
        self, data, ylabel="", ax=None, fig=None, categories=ages, colors=age_colors
    ):
        if ax is None:
            fig, ax = plt.subplots()
        boxes = ax.boxplot(
            [data[i] for i in categories],
            widths=0.75,
            showfliers=False,
            zorder=0,
            patch_artist=True,
    ...
    def corr_drift_rate_to_behavior(self, corr_matrices, performance_metric="CRs"):
        ...
        for age, color in zip(ages, age_colors):
            performance = [
                self.circle_data[mouse]["Reversal"].sdt[performance_metric]
                for mouse in self.meta["grouped_mice"][age]
            ]
            rates = [drift_rates[mouse] for mouse in self.meta["grouped_mice"][age]]
        ...
        rates = [drift_rates[mouse] for mouse in self.meta["mice"]]
    ...
    def plot_all_behavior(
        ...
        for age in ages:
            for row, mouse in enumerate(self.meta["grouped_mice"][age]):
                for border, session in zip(
                    borders, self.meta["session_types"]["circletrack"]
                ):
    ...
    def get_learning_rate_mouse(self, mouse, performance, performance_metric):
        data = performance.sel(
            mouse=mouse, session="Reversal", metric=performance_metric
        ).values.tolist()
    ...
    def get_learning_rates(self, performance_metric, window=6, strides=2):
        ...
        rates = dict()
        for mouse in self.meta["mice"]:
            rates[mouse] = self.get_learning_rate_mouse(
                mouse, performance, performance_metric
            )

        return rates

    def corr_drift_rate_to_learning_rate(
        self, corr_matrices, performance_metric, window=6, strides=2
    ):
        learning_rates = self.get_learning_rates(
            performance_metric, window=window, strides=strides
        )

        drift_rates_grouped = self.compare_drift_rates(corr_matrices, show_plot=False)

        fig, ax = plt.subplots()
        learning_rates_grouped = {
            age: [learning_rates[mouse] for mouse in self.meta["grouped_mice"][age]]
            for age in ages
        }
        ...
        for age, color in zip(ages, age_colors):
            ax.scatter(
                drift_rates_grouped[age], learning_rates_grouped[age], color=color
            )
        ...
        learning_rates = (
            learning_rates_grouped["young"] + learning_rates_grouped["aged"]
        )
    ...
    def PV_corr_all_mice(self, nbins=26, normalize_by_occ=True):
        ...
        for mouse in self.meta["mice"]:
            ...
            corr_matrices[mouse] = self.session_pairwise_PV_corr_efficient(
                mouse, nbins=nbins, normalize_by_occ=normalize_by_occ
            )
    ...
    def session_pairwise_PV_corr_efficient(
        self, mouse, nbins=26, normalize_by_occ=True, corr="spearman"
    ):
        pfs = {}
        for session in self.meta["session_types"]["lineartrack"]:
            try:
                pfs[session] = self.get_directional_pfs(
                    mouse, session, nbins=nbins, normalize_by_occ=normalize_by_occ
                )
            except:
                pass
        corr_fun = spearmanr if corr == "spearman" else pearsonr

        shape = (len(session_types["lineartrack"]), len(session_types["lineartrack"]))
        corr_matrix = {direction: nan_array(shape) for direction in directions}
        for i, session_pair in enumerate(
            product(session_types["lineartrack"], repeat=2)
        ):
            ...
            if same_session:
                try:
                    ...
                    for direction in directions:
                        even, odd = [
                            split_pfs[trial_type][direction].T
                ...
                try:
                    trimmed_map = np.asarray(
                        self.get_cellreg_mappings(
                            mouse, session_pair, detected="everyday"
                        )[0]
                    )

                    for direction in directions:
                        s1, s2 = [
                            pfs[session][direction][neurons].T
    ...
    def session_pairwise_PV_corr(
        self, mouse, nbins=51, normalize_by_occ=True, corr="spearman"
    ):
        shape = (len(session_types["lineartrack"]), len(session_types["lineartrack"]))
        corr_matrix = {direction: nan_array(shape) for direction in directions}
        for i, session_pair in enumerate(
    ...
    def PV_corr(
        self, mouse, sessions, nbins=51, normalize_by_occ=True, corr="spearman"
    ):
        same_session = sessions[0] == sessions[1]
        if same_session:
            pfs = self.get_split_trial_pfs(
                mouse, sessions[0], nbins=nbins, normalize_by_occ=normalize_by_occ
            )
        else:
            pfs = self.align_pfs(
                mouse, sessions, nbins=nbins, normalize_by_occ=normalize_by_occ
            )[0]
    ...
    def align_pfs(self, mouse, sessions, nbins=51, normalize_by_occ=True):
        trimmed_map = np.asarray(
            self.get_cellreg_mappings(mouse, sessions, detected="everyday")[0]
        )

        pfs = {
            session: self.get_directional_pfs(
                mouse,
                session,
                nbins=nbins,
                normalize_by_occ=normalize_by_occ,
        ...
        peaks = {
            direction: np.argmax(pfs[sessions[0]][direction], axis=1)
            for direction in directions
        }
    ...
    def get_split_trial_pfs(
        ...
        for trial_offset, trial_type in zip([0, 2], ["even", "odd"]):
            for direction in directions:
                trials_to_include = range(
                    start_trial[direction] + trial_offset,
        ...
        if show_plot:
            orders = {
                direction: np.argsort(np.argmax(pfs["even"][direction], axis=1))
                for direction in directions
            }
            ...
            for row, direction in zip(axs, directions):
                for ax, trial_type in zip(row, ["even", "odd"]):
                    ax.imshow(
                        pfs[trial_type][direction][orders[direction]], aspect="auto"
                    )
    ...
    def get_directional_pfs(
        self, mouse, session_type, nbins=51, neurons=None, normalize_by_occ=True
    ):
        """
